...
2023.09.18 修复获取ck脚本失效的问题，预约直播功能失效，待修复
...
function score() {
	return new Promise(resolve => {
		const signinRequest = {
			url: `https://7.wawo.cc/api/account/wx/member/assets`,
			//请求头, 所有接口通用
			headers: {
				'user-agent':
					'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF XWEB/8391',
...
function Env(name, opts) {
	...
	return new (class {
		...
		lodash_set(obj, path, value) {
			...
			path.slice(0, -1).reduce((a, c, i) => (Object(a[c]) === a[c] ? a[c] : (a[c] = Math.abs(path[i + 1]) >> 0 === +path[i + 1] ? [] : {})), obj)[path[path.length - 1]] =
				value;
			return obj;
		}
