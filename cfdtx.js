  ...
  return (
    ...
    (function (r) {
      ...
      !(function () {
        ...
        for (var i = 2, n = 0; n < 64;)
          t(i) && (n < 8 && (a[n] = e(r.pow(i, 0.5))), (c[n] = e(r.pow(i, 1 / 3))), n++),
            i++;
      })();
      var h = [],
        l = (s.SHA256 = o.extend({
    ...
    (function () {
      function r () {
        ...
        (t[0] =
          (c[0] + ((c[7] << 16) | (c[7] >>> 16)) + ((c[6] << 16) | (c[6] >>> 16))) | 0),
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      $.info = {};
      $.money = 0;
      UA = `jdpingou;iPhone;4.13.0;14.4.2;${randomString()};network/wifi;model/iPhone10,2;appBuild/100609;ADID/00000000-0000-0000-0000-000000000000;supportApplePay/1;hasUPPay/0;pushNoticeIsOpen/1;hasOCPay/0;supportBestPay/0;session/${Math.random * 98 + 1
...
async function cfd () {
  try {
    ...
    if (nowTimes.getMinutes() === 59) {
      ...
      let starttime = $.isNode()
        ? process.env.CFD_STARTTIME
          ? process.env.CFD_STARTTIME * 1
          : 60
        : $.getdata('CFD_STARTTIME')
          ? $.getdata('CFD_STARTTIME') * 1
          : 60;
      if (nowtime < 59) {
      ...
      async (err, resp, data) => {
        try {
          if (err) {
            ...
            data.hongbao.forEach(async item => {
              if (item.dwStockNum) {
                console.log(`100元库存：${item.dwStockNum}`);
                let cashOutRes = await cashOut();
                console.log(
                  `兑换结束时间 ${new Date().Format('yyyy-MM-dd hh:mm:ss | S')}`
                );
                if (cashOutRes.iRet === 0) {
                  $.money = item.ddwPrice / 100;
                  console.log(`提现成功：获得${$.money}元`);
                } else {
                  console.log(`提现失败：${cashOutRes.sErrMsg}`, flag);
                  await sleep(1800);
                  await userCashOutState();
                }
              } else {
                console.log(`提现失败：${item.ddwPrice / 100}元库存不足`, flag);
                if (flag < 7) {
                  flag++
                  await userCashOutState();
                }
...
function cashOut () {
  return new Promise(resolve => {
    ...
    $.get(
      taskUrl(
        `user/ExchangePrize`,
        `dwType=3&dwLvl=3&ddwPaperMoney=100000&strPoolName=jxcfd2_exchange_hb_202110&strPhoneID=${token['phoneid']}&strPgUUNum=${token['farm_jstoken']}`
      ),
      (err, resp, data) => {
        try {
          if (err) {
  ...
  if (_0x5108['xFLNEr'] === undefined) {
      ...
      _0xfc2aa4['atob'] ||
        (_0xfc2aa4['atob'] = function (_0x509ed4) {
          ...
          for (
            ...
            ~_0x42fadc &&
              ((_0x5a7e73 = _0x5f2c3c % 0x4 ? _0x5a7e73 * 0x40 + _0x42fadc : _0x42fadc),
                _0x5f2c3c++ % 0x4)
    ...
    var _0x503f7f = function (_0x517424, _0x3cb8bc) {
      ...
      for (
        ...
        _0x19ba11 +=
          '%' +
          ('00' + _0x517424['charCodeAt'](_0x2212a4)['toString'](0x10))['slice'](-0x2);
      }
